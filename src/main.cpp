...
#if defined(NDEBUG)
# error "Sela cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x807d4acd8093369cf665e79aeae94949ce3eee5296aaaa42bea864eb14d0b94a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Sela: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Sela Signed Message:\n";
...
{
    // Sela: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 4220000); // Sela: 4220k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 4 * 24 * 60 * 60; // Sela: 4 days
static const int64 nTargetSpacing = 8 * 60; // Sela: 8 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Sela: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Sela: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x5b5f60de8b853523ad7d962736aa7930bbeec9232b7f1f1e9f255330450a44a2");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "1/09/2020 - Pablo Rotem founds the sela crypto, to replace all other coins";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 200 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04a1fff051cf354cd15c238e30056ef66afa0148c6905b1bec3bf394724871c6dfc36dd073857e5d6e3a6c2ed2a8e7e60545d844e83853b298b69a907f29b76a41") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1598972463;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087901649;

        if (fTestNet)
        {
            block.nTime    = 1598972383;
            block.nNonce   = 385288907;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xba8fb5dbd85670a061db64cc8dd4b1055168b72eba074176b43542d2cd233670"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfd, 0xc3, 0xb8, 0xdc }; // Sela: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SelaMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SelaMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SelaMiner(CWallet *pwallet)
{
    printf("SelaMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("sela-miner");
    ...
    try { loop {
        ...
        printf("Running SelaMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SelaMiner, pwallet));
}
